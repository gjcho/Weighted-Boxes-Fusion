    ...
    for label in filtered_boxes:
        ...
        for i in range(len(new_boxes)):
            ...
            if conf_type == 'box_and_model_avg':
                ...
                weighted_boxes[i, 1] = np.clip(weighted_boxes[i, 1] *  clustered_boxes[idx, 2].sum() /(weights1.sum()/numBoxes1 + weights2.sum()/numBoxes2), 0, 1) #weights.sum()
                ...
                weighted_boxes[i, 1] = np.clip(weighted_boxes[i, 1] * len(clustered_boxes) / (weighted_boxes[i, 2] + weights[mask].sum()), 0, 1)
            elif conf_type == 'max':
